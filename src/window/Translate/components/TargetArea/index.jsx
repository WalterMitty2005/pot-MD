...
export default function TargetArea(props) {
    ...
    const [markdownRender] = useConfig('translate_markdown_render', true);
    const [markdownForce, setMarkdownForce] = useState(null);
    ...
    // whether this instance is set to manual mode (auto translate disabled in service config)
    ...
    useEffect(() => {
        ...
        if (
            ...
            // check per-instance auto translate switch, default on
            store.get(`service_auto_translate@${currentTranslateServiceInstanceKey}`).then((v) => {
                if (v === false) {
                    setManualMode(true);
                    return;
                }
    ...
    // whether the string result should be displayed with the markdown renderer
    // null in markdownForce means "auto detect", otherwise the user toggled it manually
    const useMarkdown =
        markdownRender !== false &&
        typeof result === 'string' &&
        result !== '' &&
        (markdownForce === null ? looksLikeMarkdown(result) : markdownForce);
    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* content collapse */}
                <div className='flex'>
                    {manualMode && result === '' && error === '' && !isLoading && (
                        <Button
                            size='sm'
                            variant='light'
                            className='h-[20px] text-default-500'
                            onPress={() => {
                                translate();
                            }}
                        >
                            {t('translate.manual_translate')}
                        </Button>
                    )}
                    <Button
                        size='sm'
                ...
                <div ref={boundRef}>
                    ...
                    >
                        <ButtonGroup>
                            ...
                            <Tooltip
                                content={
                                    useMarkdown ? t('translate.markdown_view_off') : t('translate.markdown_view_on')
                                }
                            >
                                <Button
                                    isIconOnly
                                    ...
                                    size='sm'
                                    isDisabled={
                                        typeof result !== 'string' || result === '' || markdownRender === false
                                    }
                                    onPress={() => {
                                        setMarkdownForce(!useMarkdown);
                                    }}
                                >
                                    {useMarkdown ? (
                                        <TbMarkdownOff className='text-[16px]' />
                                    ) : (
