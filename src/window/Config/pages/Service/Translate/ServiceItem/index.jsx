...
export default function ServiceItem(props) {
    ...
    // whether this instance should be called automatically in translate window (manual translate when off)
    const [autoTranslate, setAutoTranslate] = useConfig(`service_auto_translate@${serviceInstanceKey}`, true);
    ...
    return serviceSourceType === ServiceSourceType.PLUGIN && !(serviceName in pluginList) ? (
        ...
        serviceInstanceConfig !== null && (
            <div className='bg-content2 rounded-md px-[10px] py-[20px] flex justify-between'>
                ...
                <div className='flex'>
                    <Tooltip content={t('config.service.auto_translate_tooltip')} placement='top'>
                        <Switch
                            size='sm'
                            color='warning'
                            thumbIcon={<TbPower className='text-[10px]' />}
                            isSelected={autoTranslate !== false}
                            onValueChange={(v) => {
                                setAutoTranslate(v);
                            }}
                        />
                    ...
                    <Spacer x={2} />
                    <Tooltip content={t('config.service.display_tooltip')} placement='top'>
                        <Switch
                            size='sm'
                            color='success'
